...
| `create_campaign` / `create_adset` | authoring · `build_create_{campaign,adset}_plan` (MCP `propose_create_campaign` / `propose_create_adset`; no CLI proposer) → `apply-authoring` | campaign / adset | create-only | **Grounded + hard apply-time gate** + `review_authoring_plan`; forced **PAUSED**; net-new cites zero sample → abstain → approved create **blocked** unless the operator drops `requires_grounding` (no MCP tool drops it yet — see `mcp-local-approval-gate`). |
| `create_ad` | authoring · MCP `propose_create_ad` (net-new, existing creative id) / `propose-duplicate-ad` (proven source) → `apply-authoring` | ad | create-only | Net-new from an existing creative id cites zero sample → abstain → **blocked** unless overridden; a **duplicate** grounds on the **source ad's** own metric (proven winner → executable); forced **PAUSED**. |
